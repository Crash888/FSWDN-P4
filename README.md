...
- Speakers need to be created before they can be assigned to a session.  When a session is created a check you need to provide the key to the speaker record to assiogn the speaker to the session.  If the speaker exists then the session key is added to the sessionKeysToSpeak list.

- I didn't want to rely on any particular field in the speaker entity for the key so the speaker key is generated by the system.

- Additional Endpoints: 
  - 'updateSpeaker' can be used to update additional speaker information
  - 'createSpeaker' used to create new speakers so they can be added to sessions
...
### Task 4: Add a Task - Featured Speaker
  ...
  For this task logic was added to the createSession Endpoint to create a task to check if the speaker was speaking at any other sessions in the conference.  

  The task queries the datastore for the conference sessions by the speaker and if more than one is found then a Memcache entry with the key 'CONFERENCE\_FEATURED\_SPEAKERS'.
